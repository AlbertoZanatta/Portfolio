...
//Int for discriminating between using / not using a normal map
var usingNormalMap = 0;
...
var sphereReflectionFragmentShader = getShaderSource("sphere-reflection-fragment-shader") + getShaderSource("common");
var sphereRefractionFragmentShader = getShaderSource("sphere-refraction-fragment-shader") + getShaderSource("common");
var sphereFresnelFragmentShader = getShaderSource("sphere-fresnel-fragment-shader") + getShaderSource("common");
var sphereChromaticFragmentShader = getShaderSource("sphere-chromatic-fragment-shader") + getShaderSource("common");

//Array of programs for the rendering of the sphere
var spherePrograms = [];
//Index of the current program being use to render the sphere
var currentSphereProgram = 0; //0->Reflection, 1->Refraction, 2->Fresnel effect, 3->Chromatic dispersion

//Parameters needed for the calculation of refraction, fresnel effect and chromatic dispersion
var fresnelScale = 1;
var fresnelBias = 0;
var fresnelPower = 5;
var etaRatio = 1.0003 / 1.5;

var etaRatioRed = 1.0003 / 1.513;
var etaRatioGreen = 1.0003 / 1.519;
var etaRatioBlue = 1.0003 / 1.528;
var etaRatios = vec3(etaRatioRed, etaRatioGreen, etaRatioBlue);
...
function init() {
    ...
    //Load shaders and create the different programs that will be used in the render function
    quadProgram = createProgram(gl, quadVertexShader, quadFragmentShader);
    var reflectionProgram = createProgram(gl, sphereVertexShader, sphereReflectionFragmentShader);
    reflectionProgram.id = "reflection";
    spherePrograms.push(reflectionProgram);

    var refractionProgram = createProgram(gl, sphereVertexShader, sphereRefractionFragmentShader);
    refractionProgram.id = "refraction";
    spherePrograms.push(refractionProgram);

    var fresnelProgram = createProgram(gl, sphereVertexShader, sphereFresnelFragmentShader);
    fresnelProgram.id = "fresnel";
    spherePrograms.push(fresnelProgram);

    var chromaticProgram = createProgram(gl, sphereVertexShader, sphereChromaticFragmentShader);
    chromaticProgram.id = "chromatic";
    spherePrograms.push(chromaticProgram);
...
function render() {
    ...
    var sphereProgram = spherePrograms[currentSphereProgram];
    ...
    //Bind and set the other uniform variables: cubeMap, normalMap, eye position and the parameters needed for the calculation of the fresnel effect
    ...
    gl.uniform1i(gl.getUniformLocation(sphereProgram, "usingNormalMap"), usingNormalMap);

    initSpecific(sphereProgram);
...
function setMenus() {
    ...
    nmm.addEventListener("click", function () {
        var selectedIndex = nmm.selectedIndex;
        if (selectedIndex == 0)
        {
            usingNormalMap = 0;
        ...
        {
            usingNormalMap = 1;
            currentNormalMap = nmm.selectedIndex - 1;
        }
    ...
    var spm = document.getElementById("sphereprogram_menu");
    spm.addEventListener("click", function () {
        currentSphereProgram = spm.selectedIndex;
        if (!moveCamera) {
            render();
        }
    });
...
function setSliders() {
    ...
    sliderFs.addEventListener('input', function () {
        fresnelScale = sliderFs.value;
        document.getElementById("fresnelScale").innerHTML = "" + fresnelScale;
        if (!moveCamera) {
            render();
        }
    }, false);

    var sliderFb = document.getElementById("fb");
    sliderFb.addEventListener('input', function () {
        fresnelBias = sliderFb.value;
        document.getElementById("fresnelBias").innerHTML = "" + fresnelBias;
        if (!moveCamera) {
            render();
        }
    }, false);
    ...
    var sliderFp = document.getElementById("fp");
    sliderFp.addEventListener('input', function () {
        fresnelPower = sliderFp.value;
        document.getElementById("fresnelPower").innerHTML = "" + fresnelPower;
        if (!moveCamera) {
            render();
        }
    }, false);
...
function initSpecific(sphereProgram)
{
    if(sphereProgram.id == "reflection")
    {
        return;
    }
    
    else if(sphereProgram.id == "refraction")
    {
        gl.uniform1f(gl.getUniformLocation(sphereProgram, "etaRatio"), etaRatio);
        return;
    }

    else if (sphereProgram.id == "fresnel")
    {
        gl.uniform1f(gl.getUniformLocation(sphereProgram, "fresnelScale"), fresnelScale);
        gl.uniform1f(gl.getUniformLocation(sphereProgram, "fresnelBias"), fresnelBias);
        gl.uniform1f(gl.getUniformLocation(sphereProgram, "fresnelPower"), fresnelPower);
        gl.uniform1f(gl.getUniformLocation(sphereProgram, "etaRatio"), etaRatio);
        return;
    }

    else if (sphereProgram.id == "chromatic")
    {
        gl.uniform1f(gl.getUniformLocation(sphereProgram, "fresnelScale"), fresnelScale);
        gl.uniform1f(gl.getUniformLocation(sphereProgram, "fresnelBias"), fresnelBias);
        gl.uniform1f(gl.getUniformLocation(sphereProgram, "fresnelPower"), fresnelPower);
        gl.uniform3fv(gl.getUniformLocation(sphereProgram, "etaRatios"), etaRatios);
        return;
    }
}
